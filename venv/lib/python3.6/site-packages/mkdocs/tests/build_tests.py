...
class BuildTests(PathAssertionMixin, LogTestCase):
    ...
    @tempdir(files={'test.md': 'page content'})
    def test_populate_page(self, docs_dir):
        cfg = load_config(docs_dir=docs_dir)
        file = File('test.md', cfg['docs_dir'], cfg['site_dir'], cfg['use_directory_urls'])
    ...
    @tempdir(files={'test.md': 'new page content'})
    @mock.patch('io.open', side_effect=IOError('Error message.'))
    ...
    def test_build_page(self, site_dir):
        ...
        files = Files([File('test.md', cfg['docs_dir'], cfg['site_dir'], cfg['use_directory_urls'])])
        nav = get_navigation(files, cfg)
    ...
    #     self.assertEqual(
    #         cm.output,
    #         ["INFO:mkdocs.commands.build:Page skipped: 'test.md'. Generated empty output."]
    #     )
    ...
    def test_build_page_dirty_modified(self, site_dir, docs_dir, mock_write_file):
        cfg = load_config(docs_dir=docs_dir, site_dir=site_dir, nav=['test.md'], plugins=[])
        files = Files([File('test.md', cfg['docs_dir'], cfg['site_dir'], cfg['use_directory_urls'])])
        nav = get_navigation(files, cfg)
        page = files.documentation_pages()[0].page
        # Fake populate page
